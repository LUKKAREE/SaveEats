...
export interface DashboardStats {
  ...
  /**
   * จำนวน "ชุดอาหาร" ที่ถูกรับไปแล้วจริง สะสมตั้งแต่เปิดระบบ (RQ-053)
   *
   * *** ทำไมเป็นชุด ไม่ใช่กิโลกรัม ***
   * ร้านอาหารริมทางไม่มีตาชั่ง และไม่มีคอลัมน์น้ำหนักในฐานข้อมูล
   * ถ้าแปลงเป็นกิโลกรัมต้องเดาน้ำหนักต่อชุดเอาเอง ซึ่งเป็นตัวเลขที่อธิบายที่มาไม่ได้
   * จำนวนชุดเป็นสิ่งเดียวที่ตรวจสอบย้อนกลับได้ทุกรายการ
   *
   * *** ทำไมนับเฉพาะ completed ***
   * confirmed / waiting = จองแล้วแต่ยังไม่ไปรับ อาจหมดอายุแล้วถูกทิ้งอยู่ดี
   * expired / cancelled = ไม่มีใครเอาไป กลายเป็นขยะ
   * เหลือแค่ completed ที่พูดได้เต็มปากว่าอาหารนี้ไม่ถูกทิ้งแน่นอน
   */
  foodSavedCount: number;
  /** มูลค่ารวมของอาหารชุดข้างบน หน่วยเป็นบาท (ราคาถูกล็อกไว้ตั้งแต่ตอนจอง) */
  foodSavedValue: number;
