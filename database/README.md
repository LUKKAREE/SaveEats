...
### ต้องรัน `migration_04` ไหม

| สถานการณ์ | ทำอย่างไร |
|---|---|
| เพิ่ง import `schema.sql` ใหม่ | **ไม่ต้องรัน** เพราะ `schema.sql` มีของใหม่ครบแล้ว |
| มีฐานข้อมูล `saveeats` อยู่แล้วและไม่อยากให้ข้อมูลหาย | **ต้องรัน** ไม่งั้นหน้าการแจ้งปัญหาจะพัง |

วิธีรัน : เปิด phpMyAdmin → **คลิกฐานข้อมูล `saveeats` ทางซ้ายก่อน** → แท็บ **SQL** →
วางเนื้อหาทั้งไฟล์ → กด **Go** (ข้อมูลเดิมไม่หาย และรันซ้ำได้ปลอดภัย)
